...
function FormAddQuestions() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      setTimeout(() => navigate('/commentDetail',{ id:id }), 1000); 
    } catch (error) {
      ...
      <div className='container'>
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className='mt-3'>
            ...
            <button className='text-light btn btn-outline-secondary ms-2' type="button" onClick={() => navigate('/commentDetail',{ id: id })}>Back</button>
          </div>
        </form>
      </div>
