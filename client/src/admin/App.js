...
export default function App() {
  ...
  const getRoutes = (allRoutes) =>
    allRoutes.map((route) => {
      ...
      if (route.route) {
        return <Route path={route.route} element={route.component} key={route.key} />; // Sử dụng `element` thay vì `component`
      }
  ...
  return direction === "rtl" ? (
    <CacheProvider value={rtlCache}>
      <ThemeProvider theme={themeRTL}>
        ...
        <Routes>
          {getRoutes(routes)}
          <Route path="*" element={<Navigate to="/dashboard" replace />} /> {/* Chỉnh sửa ở đây */}
        </Routes>
      </ThemeProvider>
    </CacheProvider>
  ) : (
