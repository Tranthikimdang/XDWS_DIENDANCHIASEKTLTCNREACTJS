  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        ...
        const questionsDoc = await getDoc(questionsDocRef);
        if (questionsDoc.exists()) {
          setData(questionsDoc.data());
          setImagePreview(questionsDoc.data().image || "");
        } else {
      ...
      <div className="container">
        <form onSubmit={handleSubmit(onSubmit)} method="post" encType="multipart/form-data">
          ...
          <div className="d-flex justify-content mt-3">
            ...
            <button className="btn btn-outline-secondary" type="button" onClick={() => navigate("/admin/questions")}>Trở lại</button>
          </div>
        </form>
      </div>
      <Snackbar
