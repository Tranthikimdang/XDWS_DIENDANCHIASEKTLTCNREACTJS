...
import authorsBannerData from "./data/authorsBannerData";
import ConfirmDialog from './data/FormDeleteBanner';
...
function Banner() {
  const { columns } = authorsBannerData;
  ...
  useEffect(() => {
    const fetchBanners = async () => {
      ...
      try {
        ...
        setRows(BannersData); // Lưu dữ liệu vào state
      } catch (error) {
  ...
  const confirmDelete = async () => {
    try {
      ...
      const BannerRef = doc(db, "Banners", deleteId);
      await deleteDoc(BannerRef); // Thực hiện xóa bài viết từ Firestore
  ...
  const handleApprove = async (id) => {
    try {
      const BannerRef = doc(db, "Banners", id); // Tạo DocumentReference
      await updateDoc(BannerRef, { isApproved: 1 }); // Cập nhật trường isApproved thành 1
        ...
        <VuiBox mb={3}>
          <Card>
            <VuiBox display="flex" justifyContent="space-between" alignItems="center" mb="22px">
              ...
              <Link to="/admin/formaddbanner">
                <button className='text-light btn btn-outline-info' onClick={handleAddBannerSuccess}>
                  <svg
                        ...
                        return {
                          ...
                          action: (
                            <div className="action-buttons">
                              <Link to={`/admin/formviewBanner/${row.id}`}>
                              ...
                              <Link to={`/admin/formeditBanner/${row.id}`}>
