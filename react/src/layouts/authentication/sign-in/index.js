...
import { useState } from "react";
import { useHistory, Link, useLocation } from "react-router-dom"; // Import useLocation
    ...
    try {
      ...
      if (user) {
        ...
        if (user.role !== "admin") {
          alert("Bạn không có quyền admin."); // Thông báo nếu không phải admin
          localStorage.removeItem("user");
          navigate.push("/authentication/sign-in"); // Có thể điều hướng đến trang đăng nhập hoặc trang khác
        } else {
          localStorage.setItem("user", JSON.stringify(user));
          // Chuyển hướng đến dashboard
          navigate.push("/dashboard");
